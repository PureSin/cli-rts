...
export class TimelineControls {
    ...
    // Playback state
    rafId = null;
    playbackWallStart = 0; // performance.now() when playback began
    playbackEventStart = 0; // events[cursor].ts when playback began
    pollTimer = null;
    onScrubCb = null;
    onLiveCb = null;
    keyHandler = null;
    ...
    onScrub(cb) { this.onScrubCb = cb; }
    onLive(cb) { this.onLiveCb = cb; }
    start() {
        this.fetchEvents();
    ...
    startPlayback() {
        ...
        const tick = () => {
            ...
            while (this.cursor + 1 < this.events.length &&
