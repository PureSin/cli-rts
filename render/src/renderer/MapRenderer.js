...
export class MapRenderer {
    ...
    rebuild(state) {
        ...
        const roots = Object.values(state.map.regions).filter((r) => !r.parentId);
        for (const root of roots) {
            this.drawRegionTree(state, root);
        }
    }
    drawRegionTree(state, region) {
        this.drawRegion(region);
        for (const childId of region.children) {
            const child = state.map.regions[childId];
            if (child)
                this.drawRegionTree(state, child);
        }
    }
    drawRegion(region) {
        ...
        if (width < 2 || height < 2)
            return;
        const terrain = (region.terrain ?? "source");
