...
export class UnitPool {
    ...
    syncUnit(unit, playerColor, playerStatus, player, activeIds) {
        ...
        if (!entry) {
            ...
            if (player !== null) {
                ...
                renderer.container.on("pointerenter", (e) => {
                    if (capturedEntry.playerRef)
                        this.tooltip?.show(capturedEntry.playerRef, e.clientX, e.clientY);
                });
        ...
        for (const [id, entry] of this.pool) {
            ...
            if (this.labelOverlay) {
                ...
                if (entry.playerRef && this.tooltip) {
                    ...
                    this.labelOverlay.setHoverable(id, (sx, sy) => { if (e.playerRef)
                        t.show(e.playerRef, sx, sy); }, () => t.hide());
                }
            }
        }
    }
